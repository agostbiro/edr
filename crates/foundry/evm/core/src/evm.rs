...
use crate::{
    backend::CheatcodeBackend, constants::DEFAULT_CREATE2_DEPLOYER_CODEHASH, evm_env::EnvMut,
    EnvMut, InspectorExt,
};

pub type EvmContext<'db> = EthEvmContext<&'db mut dyn CheatcodeBackend>;
...
/// A list of features that can be enabled or disabled in the
/// [`FoundryHandler`]. This is used to conditionally override certain execution
/// paths in the EVM.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum Features {
    /// Enables routing certain `CREATE2` invocations through the
    /// `CREATE2_DEPLOYER`.
    ///
    /// If [`InspectorExt::should_use_create2_factory`] returns `true`, the
    /// standard `CREATE2` hook is overridden with a `CALL` frame targeting
    /// the deployer. The handler tracks these overridden frames and, in the
    /// `insert_call_outcome` hook, inserts the decoded contract address
    /// directly into the EVM interpreter.
    Create2Factory,
}
    ...
    fn first_frame_init(
            ...
            if let FrameInput::Create(inputs) = &frame_input {
                    ...
                    && inputs.init_code.starts_with(&EOF_MAGIC_BYTES)
                {
                    let frame_input = FrameInput::EOFCreate(Box::new(EOFCreateInputs::new(
                        ...
                        inputs.gas_limit,
                        EOFCreateKind::Tx {
                            initdata: inputs.init_code.clone(),
                        },
                    )));
        ...
        if self.is_enabled(Features::Create2Factory) {
            if let FrameInput::Create(inputs) = &frame_input {
                ...
                if !self
                    .inspector()
                    .should_use_create2_factory(evm.ctx(), inputs)
                {
                    return Self::Frame::init_first(evm, frame_input);
                }
                ...
                if let Ok(FrameOrResult::Item(frame)) = &mut frame_or_result {
                    self.inspector()
                        .initialize_interp(&mut frame.interpreter, evm.ctx());
    ...
    fn frame_return_result(
                ...
                .is_some_and(|(depth, _)| *depth == evm.ctx().journaled_state.depth)
        {
            ...
            if let FrameResult::Call(outcome) = &result {
                ...
                self.inspector()
                    .call_end(evm.ctx(), &call_inputs, &mut outcome);
...
pub fn new_evm_context<'db>(
    db: &'db mut dyn CheatcodeBackend,
    env: &EnvMut<'_>,
