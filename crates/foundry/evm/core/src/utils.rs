...
pub fn apply_chain_and_block_specific_env_changes<N: Network>(
    env: &mut EvmEnv<BlockEnv, TxEnv, CfgEnv<SpecId>>,
    ...
    // NamedChain::BinanceSmartChainTestnet => {             // https://github.com/foundry-rs/foundry/issues/9942
    //             // As far as observed from the source code of bnb-chain/bsc,
    // the `difficulty`             // field is still in use and returned by
    // the corresponding             // opcode but `prevrandao` (`mixHash`)
    // is always zero, even             // though bsc adopts the newer EVM
    // specification. This will             // confuse revm and causes
    // emulation failure.             env.evm_env.block_env.prevrandao =
    // Some(env.evm_env.block_env.difficulty.into());             return;
    //         }
    //         NamedChain::Moonbeam
    //         | NamedChain::Moonbase
    ...
    //         }
    //         c if c.is_arbitrum() => {
    //             // on arbitrum `block.number` is the L1 block which is
    // included in the             // `l1BlockNumber` field
